/*
    用const表示只读，不改变生命周期、作用域和存储位置 (运行时)
...
int g = 1; // 全局变量，全局可见，外部可访问，生命周期为程序执行时，存储在数据段
// int g; // 未初始化就存储在bss段
const int cg = 1; // 全局只读变量，全局可见，外部可访问(但声明和定义处都要extern const)，生命周期为程序执行时，存储在数据段
// const int cgg; // error: 必须初始化

static int si = 1; // 静态全局变量，全局可见，外部不可访问，生命周期为程序执行时，存储在数据段
// static int sii; // ok, bss，静态变量有静态初始化和动态初始化，这里是静态初始化
static constexpr int cei = 1; // 显示声明更好，隐式也是static，全局可见，外部不可访问，生命周期程序，存储在数据段
